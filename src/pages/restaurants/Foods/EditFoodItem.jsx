...
function EditFoodItem({ foodItem, onClose }) {
  ...
  useEffect(() => {
    if (branches?.length > 0) {
      setSelectedBranches(
        branches?.filter((branch) => foodItem.branchIds.includes(branch.value))
      );
    }
  }, [branches]);
  ...
  async function handleSubmit(event) {
    ...
    setError(null);
    setInfo(null);
    const [, error] = await putRequest(`catalogue/${foodItem._id}`, {
      ...foodItemDetails,
    });
    if (error) {
      setLoading(false);
